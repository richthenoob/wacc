...
import ic.doc.semantics.ExprNodes.Literals.ArrayLiteralNode;
import ic.doc.semantics.ExprNodes.Literals.BooleanLiteralNode;
import ic.doc.semantics.ExprNodes.Literals.CharacterLiteralNode;
import ic.doc.semantics.ExprNodes.Literals.IntLiteralNode;
import ic.doc.semantics.ExprNodes.Literals.PairLiteralNode;
import ic.doc.semantics.ExprNodes.Literals.StringLiteralNode;
...
public class Visitor extends BasicParserBaseVisitor<Node> {
    ...
    public void addTokenException(ParserRuleContext ctx, String token, String input) {
        semanticErrors.add("Semantic error at line " + ctx.getStart().getLine()
                + ":" + ctx.getStart().getCharPositionInLine() + " - Invalid character token '" + token + "' in string: " + input + ".");
    }
    ...
    private void declareFunction(BasicParser.FuncContext ctx){
        if (!ctx.IDENT().getText().equals("main")) {
            ...
            if (currentSymbolTable.getParentSymbolTable().lookup(key) == null) {
                ...
                addException(ctx, "Function '" + funcName + "' already declared in current symbol table");
            }
...
//        String name = ctx.assignLhs().getText();
    ...
    public Node visitReturn(BasicParser.ReturnContext ctx) {
        ...
        Type type = null;
        if (!main) {
            Node functionType = visitType(((BasicParser.FuncContext) curr.getParent()).type());
