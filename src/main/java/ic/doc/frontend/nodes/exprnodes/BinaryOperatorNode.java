...
public class BinaryOperatorNode extends ExprNode {
  ...
  /*  Takes in operands for binary ops that return booleans,
   *  and adds the corresponding assembly code to the label */
  ...
  private boolean exprTypesAreValid(
    ...
    if (type1IsValid && type2IsValid && !Type
        .checkTypeCompatibility(type1, type2, visitor.getCurrentSymbolTable())) {
      visitor
          .getSemanticErrorList()
          .addTypeException(
              ctx,
